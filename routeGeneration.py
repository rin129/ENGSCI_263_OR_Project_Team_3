...
# This python function generates a list of feasible routes given average weekday deamnds 
# Using this python function routes are generated and the time taken for each route
...
def routeGeneration(demand_file):

    '''
    This function generates feasible routes against time and demand constraints

    Inputs: 

            demand_file 
            a .csv file contatining the average demands of each store

    Outputs:

            routes
            a list of a list containing different feasible routes

            hours
            a list of the time taken to complete each route

            storesForLP
            a list of all the store names for the LP
    ...
    weekdayDemands = np.genfromtxt(demand_file, delimiter = ',', skip_header = 1, usecols = 2)
    ...
    return routes, hours, storesForLP
